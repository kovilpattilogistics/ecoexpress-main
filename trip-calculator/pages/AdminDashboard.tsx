...
import { subscribeToTrips, subscribeToCustomers, getUsers, saveTrip, saveCustomer, deleteCustomer, subscribeToVehicleLogs } from '../services/storageService';
import { Plus, Check, X, Search, Loader2, Sparkles, Building, Phone, Calendar, MapPin, Trash2, Edit2, UserPlus, Save, Repeat, Share2, Mail, Package, User as UserIcon, Map as MapIcon } from 'lucide-react';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onNotify }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'create' | 'customers' | 'reports' | 'map'>('overview');
  ...
  useEffect(() => {
    ...
    const unsubscribeLogs = subscribeToVehicleLogs((logs) => {
      if (isMounted.current) {
        // Process logs to get latest per driver
        const latest = new Map();
        logs.forEach(log => {
          if (!latest.has(log.driver_id)) {
            latest.set(log.driver_id, log);
          }
        });
        setVehicleLogs(Array.from(latest.values()));
      }
    });
...
To: ${trip.dropLocation}`;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex space-x-2 w-max">
          {[
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-full whitespace-nowrap text-sm font-bold transition-all shadow-sm ${activeTab === tab.id
                  ...
                  <div key={trip.id} className="bg-white rounded-2xl shadow-soft border border-green-50 overflow-hidden group hover:border-green-200 transition-colors">
                    <div className="p-5">
                      <div className="flex justify-between items-center mb-3">
                        <span className={`text-[10px] px-2.5 py-1 rounded-full font-extrabold uppercase tracking-wide ${trip.status === TripStatus.IN_TRANSIT ? 'bg-purple-100 text-purple-700' :
      ...
      {activeTab === 'map' && (
        <div className="bg-white p-4 rounded-2xl shadow-soft border border-green-50 animate-fade-in relative z-0">
          <h2 className="text-xl font-bold mb-4 text-gray-800 flex items-center">
            <MapIcon className="w-6 h-6 mr-2 text-primary" />
            Live Fleet Tracking
          </h2>
          {isLoaded ? (
            <GoogleMap
              mapContainerStyle={mapContainerStyle}
              center={vehicleLogs.length > 0 ? { lat: vehicleLogs[0].lat, lng: vehicleLogs[0].lng } : defaultCenter}
              zoom={12}
            >
              {vehicleLogs.map((log) => (
                <Marker
                  ...
                  title={drivers.find(d => d.id === log.driver_id)?.name || 'Driver'}
                  icon={{
                    url: 'https://cdn-icons-png.flaticon.com/512/3097/3097022.png', // Temporary truck icon
                    scaledSize: new window.google.maps.Size(40, 40)
                  }}
                />
              ))}
              {/* Also show active pickup/drop points */}
              {activeTrips.map(trip => (
                <React.Fragment key={trip.id}>
                  <Marker
                    position={{ lat: trip.pickupLat || 0, lng: trip.pickupLng || 0 }}
                    label={{ text: "P", color: "white", className: "bg-green-600 rounded-full p-1" }}
                  />
                  <Marker
                    position={{ lat: trip.dropLat || 0, lng: trip.dropLng || 0 }}
                    label={{ text: "D", color: "white", className: "bg-red-600 rounded-full p-1" }}
                  />      </React.Fragment>
              ))}
            </GoogleMap>
          ) : (
