...
import { MapPin, Navigation, DollarSign, Upload, CheckCircle, Clock, PlusCircle, ChevronRight, Truck, Loader2, Calendar, Share2, ArrowRight, Phone, Power } from 'lucide-react';
...
export const DriverDashboard: React.FC<DriverDashboardProps> = ({ user, onNotify }) => {
  ...
  const [view, setView] = useState<'list' | 'detail' | 'request' | 'quickLog'>('list');
  const [selectedTripId, setSelectedTripId] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isOnDuty, setIsOnDuty] = useState(true); // Default to true for now
  ...
  const handleQuickLog = async (e: React.FormEvent) => {
    ...
    const trip: Trip = {
      id: `t_quick_${Date.now()}`,
      customerName: 'Quick Logged Customer',
      customerPhone: quickPhone,
      driverId: user.id, // Assigned to self
      pickupLocation: 'Driver Location (captured)', // In real app, use Geolocation API
      dropLocation: 'Pending',
      scheduledTime: new Date().toISOString(),
      status: TripStatus.PENDING_APPROVAL,
      isDriverRequested: true,
      booking_channel: 'DRIVER_QUICK_LOG',
      createdAt: new Date().toISOString(),
      timeline: []
    } as any; // Type assertion for extra fields

    await saveTrip(trip);
    onNotify('Quick Log Saved! Admin notified.', 'success');
    setQuickPhone('');
    setView('list');
  };
...
Current Location: https://maps.google.com/?q=${selectedTrip.pickupLat},${selectedTrip.pickupLng}`;
        ...
        return (
          <div className="w-full">
            <SwipeButton
              ...
              onConfirm={() => handleStatusUpdate(TripStatus.STARTED)}
              color="bg-primary"
            />
          </div>
        );
      case TripStatus.STARTED:
        return (
          <div className="space-y-3">
            ...
            <SwipeButton
              text="Slide: Reached Pickup"
              onConfirm={() => handleStatusUpdate(TripStatus.PICKUP_REACHED)}
              color="bg-blue-600"
            />
          </div>
        );
      case TripStatus.PICKUP_REACHED:
        return (
          <div className="bg-orange-50 p-5 rounded-2xl text-center border border-orange-100 shadow-sm">
            ...
            <SwipeButton
              text="Slide: Goods Loaded"
              onConfirm={() => handleStatusUpdate(TripStatus.PICKUP_COMPLETED)}
              color="bg-orange-500"
            />
          </div>
        );
      case TripStatus.PICKUP_COMPLETED:
        return (
          <div className="space-y-3">
            ...
            <SwipeButton
              text="Slide: Start Delivery"
              onConfirm={() => handleStatusUpdate(TripStatus.IN_TRANSIT)}
              color="bg-indigo-600"
            />
          </div>
        );
      case TripStatus.IN_TRANSIT:
        return (
          <div className="space-y-3">
            ...
            <SwipeButton
              text="Slide: Reached Drop"
              onConfirm={() => handleStatusUpdate(TripStatus.DROP_REACHED)}
              color="bg-purple-600"
            />
          </div>
        );
  ...
  if (view === 'quickLog') {
    return (
      <div className="bg-white p-6 rounded-2xl shadow-soft mt-2 animate-fade-in border border-indigo-50">
        <h2 className="text-xl font-bold mb-4 text-gray-800 flex items-center"><Phone className="w-6 h-6 mr-2 text-indigo-600" /> Quick Log</h2>
        <p className="text-sm text-gray-500 mb-6">Received a call while driving? Quickly log the number here.</p>

        <form onSubmit={handleQuickLog} className="space-y-6">
          <div>
            <label className="block text-sm font-semibold text-gray-600 mb-2">Customer Phone Number</label>
            <input
              className="w-full p-6 border border-gray-200 rounded-2xl bg-gray-50 focus:ring-2 focus:ring-indigo-500 outline-none text-2xl font-mono text-center tracking-widest"
          ...
          <div className="flex space-x-3 pt-4">
            ...
            <button type="submit" className="flex-1 py-4 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-200 text-lg flex items-center justify-center">
              Save Log
            </button>
          </div>
        </form>
          ...
          <div className="p-6 space-y-8">
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
              ...
              <div className="flex items-center">
                <div className={`w-3 h-3 rounded-full mr-3 ${[TripStatus.SCHEDULED, TripStatus.PENDING_APPROVAL].includes(selectedTrip.status) ? 'bg-yellow-400 animate-pulse' :
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col mb-6 sticky top-20 z-30 bg-surface/95 backdrop-blur py-2">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center bg-white rounded-full p-1 pl-2 pr-4 shadow-sm border border-gray-100">
            <div className={`w-3 h-3 rounded-full mr-2 ${isOnDuty ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`}></div>
            <span className="text-xs font-bold text-gray-600">{isOnDuty ? 'ONLINE' : 'OFFLINE'}</span>
            <button
              onClick={() => setIsOnDuty(!isOnDuty)}
              className="ml-3 bg-gray-100 p-1.5 rounded-full hover:bg-gray-200 transition-colors"
            >
              <Power className={`w-4 h-4 ${isOnDuty ? 'text-green-600' : 'text-gray-400'}`} />
            </button>
          </div>
          <button
            onClick={() => setView('quickLog')}
            className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center shadow-lg active:scale-95 transition-transform"
          >
        ...
        {trips.length === 0 ? (
          ...
          trips.map(trip => (
            <div
              ...
              className={`bg-white rounded-2xl shadow-soft border-l-[6px] active:scale-[0.98] transition-all cursor-pointer relative overflow-hidden group ${trip.status === TripStatus.PENDING_APPROVAL ? 'border-orange-400' : 'border-primary'
                }`}
            >
              ...
              <div className="p-4 pt-3">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`text-[10px] px-2 py-1 rounded-md font-extrabold uppercase tracking-wide ${trip.status === TripStatus.SCHEDULED ? 'bg-blue-50 text-blue-600' :
